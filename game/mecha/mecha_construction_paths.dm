...
/datum/construction/reversible/mecha/phazon
	...
	custom_action(index, diff, atom/used_atom, mob/user)
		...
		switch(index)
			...
			if(2)
				if(diff==FORWARD)
					user.visible_message("[user] secures Phazon Armour Plates.", "You secure Phazon Armour Plates.")
					holder.icon_state = "phazon19"
				else
					user.visible_message("[user] pries Phason Armour Plates from [holder].", "You prie Phason Armour Plates from [holder].")
					new /obj/item/mecha_parts/part/phason_armour(get_turf(holder))
					holder.icon_state = "phason17"
			if(1)
				if(diff==FORWARD)
					user.visible_message("[user] welds Phason Armour Plates to [holder].", "You weld Phason Armour Plates to [holder].")
				else
					user.visible_message("[user] unfastens Phason Armour Plates.", "You unfasten Phason Armour Plates.")
					holder.icon_state = "phason18"
		return 1

	spawn_result()
		..()
		feedback_inc("mecha_phason_created",1)
		return
